...
const modalConta = window.bootstrap
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    ...
    .addEventListener("change", carregaDados);
});
...
async function carregaDados() {
  ...
  try {
    ...
    for (const c of data) {
      ...
      const estaPaga =
        c.data_pagamento && !c.data_pagamento.startsWith("0001-01-01");
...
async function baixar(idContasPagar) {
  if (!confirm("Deseja realmente confirmar o pagamento e baixar este título?"))
    return;
  ...
  try {
    const resp = await fetch(`/contas-pagar/pagar/${idContasPagar}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
